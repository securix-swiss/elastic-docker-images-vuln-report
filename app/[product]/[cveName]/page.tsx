...
async function loadCVEData(product: string, cveName: string): Promise<CVE | null> {
  const dataPath = path.join(process.cwd(), 'data/results', `${product}.json`);
  try {
    const fileContent = fs.readFileSync(dataPath, 'utf8');
    return JSON.parse(fileContent).cveData[cveName];
  } catch (error) {
    console.error(`Error loading file for product ${product} and cve ${cveName}:`, error);
    return null;
  }
}
    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>{product} - {cveName}</CardTitle>
